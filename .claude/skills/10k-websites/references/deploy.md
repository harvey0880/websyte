...
- Kling has no free per-job price check: every image or video call is a real charge the moment it goes in, and a job should never be submitted just to see what happens. So the honest-costs talk is about the balance being real and watched, not about a quoted price: check `query_membership_and_credits` before the session's first generation and again after any video job, and say the real numbers out loud.
- A hero video is the expensive step of the pipeline; the starting frame and supporting stills are the cheap ones. The video model choice affects both quality and capability (multi-image, chaining, subject consistency), spelled out in the cost preflighting section of `prompt-laws.md`, but not a live price comparison, since Kling doesn't expose one.
- **A brand-new free-tier Kling account starts at 0 credits.** That should already have surfaced in Phase 1's scan; if it hasn't been added to yet, nothing can generate. Direct the user to kling.ai's membership page (or the link an insufficient-credits error from `query_tasks` returns) to add credits, and check current plans there instead of trusting a number written here. Say all of this plainly so the user can plan.

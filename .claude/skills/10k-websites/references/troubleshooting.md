...
| A job comes back with a content moderation block on an innocent abstract shot | Kling's safety filter misread abstract sensory language (glowing forms, flowing liquid around a shape) as disallowed content | Check `query_membership_and_credits` first; it isn't documented whether a moderation-blocked job consumes credits, so confirm the real balance before assuming a retry is free. Then re-roll the same start frame with the prompt rewritten in plain commercial product-photography words: name the product early, describe objects not sensations, keep the same shot design |
| A job returns an "insufficient credits" error mid-build | The balance ran out between Phase 1's scan and this generation | Relay the exact message, then send the user to the top-up link the error itself returns (Kling's membership page); do not silently retry |
| A job returns a "model not supported" or "parameter error" for a combination that worked earlier | The model or its argument spec changed, or a parameter is outside that model's `allowedValues` | Re-run `who_am_i` to refresh the current model/argument specs rather than trusting cached values, fix the offending field, and retry |
| A job times out or comes back as a service error | A transient failure on Kling's side | Relay the exact message; only resubmit when the user explicitly asks, with the same parameters kept |
...
| Kling tools missing right after connecting, or `who_am_i` reports a newer `mcpVersion` than the tool list in use | A freshly added connector sometimes loads only on the next app start, or the host's cached tool list is stale | Close Claude Code completely, reopen, return to the same chat, and call `who_am_i` again to verify |
| The Kling connector shows it needs authentication | The sign-in did not complete during the add | Finish it in the connectors panel: the browser opens to Kling to authorize once, then re-verify with `who_am_i` |
| Kling tools present but generation calls fail | Connector added but not authorized, or the account balance is 0 | Re-run the sign-in flow if unauthorized; otherwise call `query_membership_and_credits` — a real 0 is a valid answer and means the account needs credits before anything can generate, not a broken connector |
| Kling was told to log out or switch accounts, and the next call fails | `logout` revoked the session immediately (`reloginRequired=true`) | Re-trigger the Kling sign-in flow right away; do not run any other Kling tool until sign-in completes, and never ask the user for a raw access or refresh token |

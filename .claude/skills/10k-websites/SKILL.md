...
**If other website skills are installed, this one governs.** A user's setup may carry other website-builder or AI-video skills alongside this one. The user asked for this skill, so the whole build runs on this skill alone: its phases, its laws, its gates, its standards. Never blend another skill's website workflow into this one, and never hand the build off to one.
...
Assume nothing about what is already set up. Some users arrive with Kling connected moments ago, because a tutorial had them connect it right before dropping this skill in. Others arrive with a completely fresh machine and nothing connected at all. Both are normal, neither is a surprise, and the scan tells you which one this is. The steps below cover whatever the scan finds missing, in order, and nothing more.

1. **Scan the system before asking the user anything.** Check each prerequisite yourself: Kling tools available (image and video generation)? `ffmpeg` runs in the terminal? Node.js installed (`node --version`; the local preview server runs through it)? Also note quietly whether Hostinger tools happen to be connected already from an earlier project. That note is for Phase 10 only, never for the report.
2. **Report the scan as a simple checklist.** A ✓/✗ line per item and the plan for the missing ones in order. The checklist covers the build tools only. When Kling is connected, call `who_am_i` to confirm it and `query_membership_and_credits` for the real balance, and name that number on the checklist so the user knows what there is to work with. **If the balance is 0 (a fresh free-tier account starts here), say so plainly right on the checklist** — nothing can generate until credits are added, and that has to be visible before any creative work starts, not discovered mid-build. If everything the build needs is present, say so, give the honest-costs talk from step 4, and go straight to declaring setup complete.
...
4. **Kling, the one manual step of setup.** Have the user create a Kling account at kling.ai if they do not have one. Then connect it as a custom connector, which is a set of clicks only the user can do: in Claude Code, the plus button, then Connectors, then Manage connectors, then Add, then Add custom connector, using the exact URL and name from Kling's own connector page (the account settings or developer page on kling.ai lists the current MCP connector URL; never guess it). Click Add, then Connect. Their browser opens to Kling; they sign in once and authorize, and the connector needs no app restart. Then wait with a clickable check-in: one option, "Done, check it," to click when they have finished. When they click it, VERIFY: call `who_am_i` (confirms the connection and lists the available models) and `query_membership_and_credits` (confirms a real balance, even if that real balance is 0). If the tools do not appear right after connecting, one full close-and-reopen of the app loads the fresh connector: have them return to this same chat and check again. If the check still fails, help them retrace the clicks instead of moving on. Once verified, give the honest-costs talk (see `references/deploy.md`), which for Kling is mostly about the balance being real and checked, since there is no free per-job price preview.
5. **Declare setup complete** with the checklist all ✓ (or the 0-credit balance flagged plainly if that's where the account stands). If the whole process ever needs an app restart, it happens in Phase 10 when hosting connects, and even there only sometimes. The Phase 1 close-and-reopen for Kling is a rare contingency, not part of the plan.
...
1. **Check the balance, and set the money picture honestly.** Kling has no free per-job price check (no `get_cost` equivalent): every `text_to_image`, `image_to_image`, `text_to_video`, and `image_to_video` call is a real charge the moment it is submitted, and Kling's own guidance is explicit that a job should never go in just to see what happens. So call `query_membership_and_credits` before this phase's first generation and tell the user the real balance. Give the road ahead in one honest line: the starting frame is the cheap step, the video after it is the expensive one, and the model for the video gets chosen together once the frame is approved. Confirm the plan out loud before anything uncertain gets submitted.
2. **The starting frame:** generate with a Kling image model (`text_to_image`, or `image_to_image` when working from a supplied photo), 16:9, 2k, composed as frame one of the motion, lit and colored in the brand's world, "no text, no logos" included. A user's real product photo can be the starting frame instead. When the subject is a real person who has agreed to appear, build a Kling Element from their photo first (see `references/prompt-laws.md`) so later calls stay on their real likeness.
3. **Inspect the image yourself before animating.** Look at it. Check for sneaked-in trademarks and logos (AI loves to add real brand marks), broken anatomy, and composition. A bad image is a cheap fix now or a whole video's credits wasted later. Then show the user: a quick yes here is cheap insurance.
4. **The model choice, after the frame is approved and before the video call goes in.** Without a live price comparison, the choice runs on capability, not a real-time price table (spelled out in `references/prompt-laws.md`): `kling-video-v3_0_turbo` is Kling's stated first choice for a single image-to-video shot and is the default; `kling-video-v3_0` and `kling-video-v3_0_omni` add a `tail_image` target, multi-image input, and Element (subject) consistency for chained or character-driven builds; `kling-video-v2_5` and `kling-video-v2_6` are the lower tier but only support 5 or 10 second durations, not this skill's proven 6-second default. Recommend one, say why, and let the user choose. For a chained journey the model was already chosen in the Creative Director's Loop; confirm it here instead of re-asking.
5. **The video** (image-to-video, 720p — Kling's current cap for its video tools, not 1080p — 6 seconds, audio disabled): write the prompt from the laws.
